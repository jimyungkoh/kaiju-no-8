...
object ProducerAsyncWithKey {
    private val logger = LoggerFactory.getLogger(ProducerAsyncWithKey::class.java.name)
    ...
    fun sendMessage(message: String) {
        try {
            KafkaProducer<String, String>(createProducerConfig()).use { producer ->
                for (key in 0..20){
                    val record = ProducerRecord(TOPIC_NAME, "$key", "$key: $message")
...
fun main() {
    ProducerAsyncWithKey.sendMessage("hello kafka!")
}